...
int main(int argc, char **argv){
  ...
  map=mmap64(NULL,(size_t)size,PROT_READ|PROT_WRITE,MAP_SHARED,fd,0);
  if((void*)-1 == map){ err(strerror(errno));exit(1);}
  ...
  while(1){
    ...
    if(m == request_magic && f < (uint64_t)1<<63 && l+f <= size){
      ...
      if(t==0){
        ...
        for(;ll>0;ll-=B){W(map+f,clip(ll));}
      } else if(t==1){
        for(;ll>0;ll-=B){R(map+f,clip(ll));}
        E(msync(map,size,MS_SYNC));
        W(&rmag,4);W("\0\0\0\0",4);W(h,8);
      }
    } else {
