...
void W(void *b,size_t l){int i; for(i=0;i<l;i+=write(1,b+i,l-i));}
void R(void *b,size_t l){int i=0; while(i<l){ int j=read(0,b+i,l-i); if(j==0){exit(0);} i+=j;}}
ssize_t s(int out_fd,int in_fd,off64_t* f, size_t l){ssize_t r = sendfile64(out_fd,in_fd,f,l); if(r==-1){err(strerror(errno));} return r;}
void WS(off64_t *f,size_t l){ssize_t i; for(i=0;i<l;i+=s(1,fd,f+i,l-i));}
...
int main(int argc, char **argv){
  ...
  while(1){
    ...
    if(m == request_magic && f < (uint64_t)1<<63 && l+f <= size){
      int64_t ll=l; off64_t fo=(off64_t)f;
      if(t==0){
        W(&rmag,4);W("\0\0\0\0",4);W(h,8);WS(&fo,ll);
      } else if(t==1){
